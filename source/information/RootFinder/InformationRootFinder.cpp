...
void InformationRootFinder::getSecondInitialGuess()		// Indexes 4, 2, 3
{
	...
	unsigned int size = 3;
	std::vector<double> initial_guess = generator_->generateNumberSeq(size, 0.001, upper_bounds_.at(1));
...
void InformationRootFinder::solve()
{
	...
	if (!solver_not_needed){
		...
		int count = 1;
		while (!solution_valid && count < rfconfig::kMaxTries){
			resetMap();
			solveSystem(getSolutionGuesses());
			solution_valid = solutionsValid();
			++count;
		}
		if (count == rfconfig::kMaxTries){
			throw GSLDivergenceException();
		}
	} else{
